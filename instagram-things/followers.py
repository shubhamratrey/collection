...
class Followers(object):
    DEFAULTS = {
        'followers': "/api/v1/friendships/{user_id}/followers/",
    }
    ...
    def get_all_followers_profiles_using_user_id(self, user_id):
        data = {
            'users': [],
            'n_request': 0,
            'count': 0
        ...
        next_max_id = None
        while looping:
            inner_users, next_max_id = self.get_followers_profiles_using_user_id(user_id=user_id,
    ...
    def get_followers_profiles_using_user_id(self, user_id, next_max_id=None, page_size=200):
        users, max_id = [], None
        params = {'max_id': next_max_id, 'count': page_size} if next_max_id else {'count': page_size}
        resp_json = self.client.get(self.DEFAULTS['followers'].format(user_id=user_id), params=params)
